...
class Gomoku():
    ...
    def gomoku_possibilities_to_tensor(self, stone_list_target, stone_list_opponent):
        """
        :param stone_list_target:
        :param stone_list_opponent:
        :return: return every possible point's gomoku_tensor
        """
        all_remain_states = [[i,j] for i in range(15) for j in range(15)]
        all_remain_states = [i for i in all_remain_states if i not in stone_list_target + stone_list_opponent]

        torch_tensor = torch.empty(0, 12, 15, 15)
        for _ in all_remain_states:
            stone_list_target_copy = list(stone_list_target)
            stone_list_target_copy.append(_)
            one_tensor = self._gomoku_to_tensor(stone_list_target_copy, stone_list_opponent)
            torch_tensor = torch.cat((torch_tensor, one_tensor), dim = 0)

        return torch_tensor, all_remain_states
