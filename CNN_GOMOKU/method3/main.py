...
class Window(QMainWindow):
    ...
    def _get_point_AI(self, push_hist_black, push_hist_white):
        weight_tensor = torch.FloatTensor([2, -3])
        # person : black, AI : white
        if self.first_turn == 0:
            target_index = 0
            tensor_point_possible_situ, possible_states = self.gomoku.gomoku_possibilities_to_tensor(push_hist_white, push_hist_black)
            tensor_point_value = model(tensor_point_possible_situ, tensor_point_possible_situ.size()[0])
            tensor_point_value = tensor_point_value.matmul(weight_tensor)
            target_point_top10 = tensor_point_value.topk(tensor_point_value.size()[0]).indices
        ...
        # person : white, AI : black
            ...
            target_index = 0
            tensor_point_possible_situ, possible_states = self.gomoku.gomoku_possibilities_to_tensor(push_hist_black,
                                                                                                     push_hist_white)
            tensor_point_value = model(tensor_point_possible_situ, tensor_point_possible_situ.size()[0])
            tensor_point_value = tensor_point_value.matmul(weight_tensor)
            target_point_top10 = tensor_point_value.topk(tensor_point_value.size()[0]).indices
